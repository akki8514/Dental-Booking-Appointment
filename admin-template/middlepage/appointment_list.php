                        ...
                        <div class="table-responsive">
                            <table class="datatable table table-stripped ">
                                ...
                                <tbody>
                                   <?php 
                                       $i = 1;
                                       $Dataappointment = "SELECT * FROM `appointment` ";
                                       $QryAppointment = mysqli_query($con,$Dataappointment);
                                       while($RowAppointment = mysqli_fetch_object($QryAppointment))
                                       {
                                        $cat = mysqli_fetch_assoc(mysqli_query($con,"SELECT * FROM `category` WHERE `cat_id` = '".$RowAppointment->cat_id."' "));
                                        $user = mysqli_fetch_assoc(mysqli_query($con,"SELECT * FROM `patient` WHERE `p_id` = '".$RowAppointment->p_id."' "));
                                       ?>
                                    <tr>
                                        <td><?php echo $i; ?></td>
                                        <td><?php echo $cat['name']; ?></td>
                                        <td><?php echo $RowAppointment->fname; ?></td>
                                        <td><?php echo $RowAppointment->lname; ?></td>
                                        ...
                                        <td>
                                         <?php 
                                         if ($RowAppointment->status=="pending") 
                                            { ?>
                                              <a href="index.php?page=appointment_action&action=accept&a_id=<?php echo $RowAppointment->a_id; ?>"><span class="fa fa-check text-success">
                                                </span></a>
                                                &nbsp;&nbsp;
                                                <a href="index.php?page=appointment_action&action=reject&a_id=<?php echo $RowAppointment->a_id; ?>"><span class="fa fa-close text-danger"></span>
                                                </a>
                                           <?php } elseif ($RowAppointment->status=="accept") 
                                           { ?>
                                            <span class="custom-badge status-green"><?php echo ucfirst($RowAppointment->status); ?>
                                                    </span>

                                            <?php } elseif ($RowAppointment->status=="reject") { ?>
                                                  <span class="custom-badge status-red"><?php echo ucfirst($RowAppointment->status); ?>
                                                    </span>                                                
                                           <?php } ?> 
